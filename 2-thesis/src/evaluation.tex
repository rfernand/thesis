...
request or responds with a fake successfull registration is
...
\label{sec:eval_lazy_maintenance}
  ...
  % TODO SUM the error in key recovery and auth request
\section{Sign-in}
...
request (or that $I$ responds fake user information) is
...
scalable when the size $N$ of the DHT increases.
  ...
  \begin{enumerate}
    \item{\textit{Probability of failure:}}
    A identified service request between a node $A$, the service $S$ and $I$ can fail (a) if $I$ does
...
information recovery request, or that $S$ not respond to the node $A$ is
    ...
    Table~\eqref{tab:p_identified_service_request} shows the probability of failure
...
having the same associated cost as before $n = (L+1)(5 \times O(log_{2b}(N)) + Q + 4)$~\ref{sec:eval_leafset}.
    ...
    \begin{align}
      ...
      n &= (L+1)(5 + 3(L+1) + L)\\
      n &= 3(L+1)^2 + 5L + 5 + L^2 + L\\
      n &= 3L^2 +6L + 3 + 5L + 5 + L^2 + L\\
      n &= 4L^2 +12L + 8 
    \end{align}
     The total cost is then
...
request or responds with a fake successfull logout is
