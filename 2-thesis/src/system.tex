...
%%The identifiers $I_i$ of the username $username$ forms the identification service $I$ for the user with username $username$.
...
%%register a user identity, to identify himself with an existing user identity,
...
$I$.
 ...
 %We can minimize the risk of this happening using an maintenance scheme for the identifiers nodes data.
...
%The user's information consists of the user's public keys and salt. This has to be done on every node in the identification service of set
...
%composed of its leafset, and of the $\frac{L}{2} +1$ nodes  to the right and left of its leafset.
...
%%%operation process. With a lazy store maitenance, there can be a large amount
...
%%%In order to build this set of nodes, $X$ can ask the farthest nodes $X_{-L/2}$
%%%and $X_{L/2}$ of its leafset for their own respective leafsets. If one of these
%%%nodes fails to answer in a timely manner, $X$ can ask the second farthest
%%%node and so on, until its immediate neighbors in the leafset. Having
%%%$\frac{L}{2} +1$ malicious neighbors on both sides is highly improbable, and
%%%practically unfeasible. For more details about this statement, please refer to
%%%our probabilistic evaluation in section~\eqref{sec:eval_lazy_maintenance}. \\
...
generate its private key. The \textit{salt} is first encrypted by a simmetric encryption algorithm
...
%and salt used to derive its new private key $U^{salt}_{new}$. 
...
Then the node $A$ needs to also register the \textit{salt} $U^{salt}$ used to
re-generate its private key. The \textit{salt} is first encrypted by a simmetric encryption algorithm
using the user' s password $Crypt(salt, SHA(password)) = E_{U^{salt}}$
To obtain the node $T$ that provides the storage of the encrypted \textit{salt},
node $A$ computes its key $K$ such that $K = SHA(U^{username})+SHA(U^{password})$.
Then the node $A$ sends a request to the node $T$ to register the
...
by a simmetric encryption algorithm like 3DES by doing $Cypt(salt,
...
the node decrypts the \textit{salt} and proceeds to derive the user private key $U^{priv}$ using the user password $U^{password}$.
