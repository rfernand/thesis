...
%It searches for honest nodes to improve the service, and uses different
%nodes as starting points in order to reach the same root node $K_{ROOT}$.

If a node $X$ wants to obtain the leafset of a node $K_{ROOT}$, a node $X$ sends a get\_leafset\_root request.
...
leafset and ends the request. After obtaining the first leafset, the starting
node ......

%% translate this
%%%procesamiento de este mensaje funciona como sigue: Cada nodo que recibe el
%%%mensaje,
%%%comprueba si el nodo ra ́ız pertenece a su propio Leaf Set, si esta condici ́
%%% on se cumple,
%%%entonces el nodo env ́ıa su propio Leaf Set y termina el mensaje. El algoritmo 2
%%%muestra
%%%como procesar este mensaje en Pastry, la funcion forward(m, k, null) forma parte de la
%%%API de Pastry, y permite detener el mensaje en el nodo local [18].
%%%
%%%Luego de obtener el primer Leaf Set, el nodo emisor ejecuta una funci ́
%%% on de filtro, que extrae $L/2$ nodos cuyo nodeID es inferior a $k$ y $L/2$ nodos cuyo
%%%nodeID es superior a $k$. Luego, consulta directamente el Leaf Set del nodo del
%%%extremo que falta para completar el LeafSet del nodo $Y_r$. Si este nodo es
%%%malicioso y no entrega su LeafSet, el nodo emisor puede intentar recuperar el
%%%Leaf Set del nodo siguiente en la lista.  
%%%
%%%La figura 5.1 muestra un ejemplo de este procedimiento. El nodo X env ́ıa un
%%%mensaje
%%%
%%%de tipo get leaf set root, el cual es recibido por el nodo Y0, que calcula de
%%%forma
%%%independiente que Y−2 ≤ k ≤ Yr , es decir que el nodo ra ́ız Yr pertenece a su
%%%propio
%%%Leaf Set, y env ́ıa su propio Leaf Set al nodo X. Luego, el nodo X aplica una
%%%funci ́
%%% on de
%%%filtro y determina que el conjunto de nodos L = {Y0, Y1} forman parte del Leaf
%%%Set del
%%%nodo Yr , por lo cual solicita directamente el Leaf Set del nodo Y1.
%%%Finalmente, el nodo X
%%%solicita el Leaf Set del nodo Y2 y completa el Leaf Set del nodo Yr .
%%%Note que el peor caso, ocurre cuando el nodo que responde el mensaje get leaf
%%%set root
%%%se encuentra en el extremo del Leaf Set del nodo malicioso, y la funci ́
%%% on de filtro extrae s ́
%%% olo
%%%L
%%% nodos. Si p es la probabilidad que un nodo sea malicioso, la probabilidad que
%%% L
%%% nodos
%%%2
%%% 2
%%%L
%%%sean maliciosos es p 2 , la cual es muy baja para valores modestos de L. El
%%%algoritmo 2
%%%muestra como implementar este procedimiento en Pastry.
%%%
...
\caption{Node $X$ asking for the leafset of the key $Y$ using only trusted nodes}
